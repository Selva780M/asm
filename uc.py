	...
	if ENTRY:		
		if user_STOCK == "NIFTY":			
			...
			if user_OPTION == "call":
				...
				st.balloons()
				new_data = {"DATE" : DATE ,"NAME": user_USER, "STOCK" : n_call.name,  "ENTRY" : float(0.00), "QTY" : int(user_LOT), "STOPLOSS" : float(0.00 - user_STOP), "TARGET" : float(0.00 + user_TARGET), "P_L" : 0, "%" : 0 }
